...
struct state_vv : state_ab// 2 x vec2d
{
    ...
    state_vv( vec2d *pA, vec2d *pB, std::function<void(vec2d,vec2d)> p_SetFunc ): pSetFunc( p_SetFunc ), pa(pA), pb(pB) { a = *pa; b = *pb; }

    virtual void setState()const { pSetFunc(a,b); }
    virtual state_ab* clone_current()const { state_vv* pVV = new state_vv(*this); pVV->a = *pa; pVV->b = *pb; return pVV; }
    virtual void update() { a = *pa; b = *pb; }
    virtual ~state_vv(){}
};
...
struct state_f : state_ab// 1 x float
{
    ...
    state_f( float *pA, std::function<void(float)> p_SetFunc ): pSetFunc( p_SetFunc ), pa(pA) { a = *pa; }

    virtual void setState()const { if( pSetFunc ) pSetFunc(a); else *pa = a; }
    virtual state_ab* clone_current()const { state_f* pFF = new state_f(*this); pFF->a = *pa; return pFF; }
    virtual void update() { a = *pa; }
    virtual ~state_f(){}
};
...
struct state_ff : state_ab// 2 x float
{
    ...
    state_ff( float *pA, float *pB, std::function<void(float,float)> p_SetFunc ): pSetFunc( p_SetFunc ), pa(pA), pb(pB) { a = *pa; b = *pb; }

    virtual void setState()const { pSetFunc(a,b); }
    virtual state_ab* clone_current()const { state_ff* pFF = new state_ff(*this); pFF->a = *pa; pFF->b = *pb; return pFF; }
    virtual void update() { a = *pa; b = *pb; }
    virtual ~state_ff(){}
};
...
struct state_vvfi : state_ab// 2 x vec2d + float + int
{
    ...
    virtual void setState()const { pSetFunc(va,vb,fa,ia); }
    ...
    virtual void update() { va = *pva; vb = *pvb; fa = *pfa; if( pia ) ia = *pia; else if( pGetInt ) ia = pGetInt(); }
    virtual ~state_vvfi(){}
};
...
struct state_ffvp : state_ab// 2 x float + void*
{
    ...
    void* pAddy;// address stored

    state_ffvp( float *pA, float *pB, void** pp_Addy ): pa(pA), pb(pB), ppAddy( pp_Addy ) { a = *pa; b = *pb; pAddy = *ppAddy; }

    virtual void setState()const { *pa = a; *pb = b; *ppAddy = pAddy; }
    virtual state_ab* clone_current()const { state_ffvp* pFF = new state_ffvp(*this); pFF->a = *pa; pFF->b = *pb; pFF->pAddy = *ppAddy; return pFF; }
    virtual void update() { a = *pa; b = *pb; pAddy = *ppAddy; }
    virtual ~state_ffvp(){}
};
