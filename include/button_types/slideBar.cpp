...
/*    marks[0].position.x = pos.x + capLen; marks[0].position.y = pos.y + b;// min mark
 ...
 //   float boxGap = 2.0f;
    ...
    valBox[1].position.x = x0 + 2.0f*boxDim; valBox[1].position.y = y0;// up rt
    valBox[2].position.x = x0 + 2.0f*boxDim; valBox[2].position.y = y0 + boxDim;// dn rt
...
void slideBar::setPosition( sf::Vector2f Pos )
{
    ...
    valStr.setPosition( valStr.getPosition() + dPos );
    minValStr.setPosition( minValStr.getPosition() + dPos );
    maxValStr.setPosition( maxValStr.getPosition() + dPos );
