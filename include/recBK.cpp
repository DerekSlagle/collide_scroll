...
{
    if( index_last >= pHistoryVec.size() ){ std::cerr << "\nsvbh"; return; }// something very bad happened
...
{
    frameCnt = dFrames*(frameCnt/dFrames);// even up any fraction
    if( frameCnt == dFrames*(pHistoryVec.size() - 1 ) ) { std::cout << "already at final state\n"; std::cerr << "\n at end already"; return; }
...
void rec::record()
{
    if( pHistoryVec.size() == 0 ){ frameCnt = 0; saveState();  }
